...
function addImage() {
  const pos = form.value.images!.length + 1
  if (pos <= 4) {
    form.value.images!.push({ image_path: '', position: pos, is_primary: false })
  }
}
...
<template>
  <div class="admin-form">
    <div class="form-header">
      ...
      <button type="button" class="btn" @click="router.push('/admin/vehicles')">← Back to list</button>
    </div>
      ...
      <div class="financing-section">
        ...
        <div class="installment-grid">
          <div v-for="term in INSTALLMENT_TERMS" :key="term.key" class="field">
            ...
            <input
              v-model.number="form.financing_options[term.key]"
              type="number"
              min="0"
              step="0.01"
              placeholder="Monthly"
          ...
          <div class="image-actions">
            <label class="upload-btn">
              <input
                ...
                @change="onFilePicked(idx, $event)"
              />
              {{ uploadingIndex === idx ? 'Uploading…' : 'Upload photo' }}
            </label>
            <label class="primary-radio">
              <input v-model="img.is_primary" type="radio" :name="'primary-' + route.params.id" /> Primary
            </label>
...
.primary-radio { font-size: 0.9rem; margin-right: 0.5rem; }
